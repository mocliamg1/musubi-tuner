...
def convert_state_dict(state_dict: dict[str, torch.Tensor], reverse: bool = False) -> tuple[dict[str, torch.Tensor], int, int]:
    ...
    if reverse:
        ...
        keys = list(state_dict.keys())
        for key in keys:
            if ("img_attn" in key or "txt_attn" in key) and "_qkv" in key and "lora_down" in key:
        ...
        keys = list(state_dict.keys())
        for key in keys:
            if ("img_attn" in key or "txt_attn" in key) and ("_q" in key or "_k" in key or "_v" in key):
                if "_q" not in key or "lora_up" not in key:  # ensure we process only once per QKV set
                    continue
